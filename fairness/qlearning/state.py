import numpy as np
from fairness.utils import expected

class State:
    '''Representation for a game state'''

    # possible actions: choose a threshold from 
    ACTIONS = ['u', 'd', 'l', 'r', '_']

    def __init__(self, a, b, w_a, w_b, u_plus, u_minus):
        '''Initializes the state given the API string passed from the game engine'''

        # calculate mean (to be used for fairness constraint checks)
        self.mean_diff = np.abs(np.mean(a) - np.mean(b))

        # calculate utility (to be used as reward)
        self.utility = w_a * expected(a, u_plus, u_minus)
